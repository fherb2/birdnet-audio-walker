...
### 1.3 Kernarchitektur-Prinzipien

#### 1.3.1 Ein Server – ein Systemzustand – ein GUI-Client
...
#### 1.3.2 Trennung von Server-Prozess und Haupt-Prozess
...
#### 1.3.3 Direkter Zugriff auf die AsyncIO-Event-Loop
...
#### 1.3.4 Beibehaltung der bestehenden Backend-Schicht
...
#### 1.5.2 Inhalt des AppState

Der AppState gliedert sich in vier Bereiche:
...
#### 1.5.4 UI-Aktualisierung bei State-Änderungen
...
### 1.6 Navigation und Seitenstruktur

#### 1.6.1 Seitenaufteilung

Das System behält die drei bestehenden Hauptseiten bei:

- **Database Overview** (Seite 1): Datenbankauswahl, Metadaten, Species-Liste
- **Audio Player** (Seite 2): Filter, Audio-Optionen, Detektionsliste, Player
- **Activity Heatmap** (Seite 3): Heatmap-Visualisierung, Filter, Dialog
...
#### 1.6.2 Seitenübergreifender State
...
### 1.7 Hintergrundtasks – Konzept für Schritt 1
...
### 2.4 Species-Liste aktualisieren
...
#### 2.7.2 Standardsortierung und Filterung
...
### 2.8 Download der Species-Liste
...
### 2.9 Sidebar: Spezies-Selektion und Navigation
...
#### 3.2.1 Spezies-Filter mit Autocomplete
...
#### 3.2.4 Confidence, Limit, Offset, Sortierung
...
#### 3.4.2 Sprach- und Geschwindigkeitsoptionen
...
### 3.8 Audio-Generierung als Hintergrundtask
...
### 3.9 Eingebetteter HTML/JS Audio-Player

Der Audio-Player wird als eingebettete HTML/JS-Komponente in die Seite eingebettet (`ui.html`). Die Architektur und das Verhalten des Players bleiben gegenüber der Streamlit-Implementierung vollständig erhalten:
...
Den **Spezies-Filter**: dieselbe Autocomplete-Komponente wie auf der Audio-Player-Seite. Der Filter kann leer sein (alle Spezies) oder eine Spezies selektiert haben. Der aktive Filter wird im AppState unter einem eigenen Schlüssel (`hm_filter_species`) gehalten, getrennt vom Audio-Player-Filter, damit beide Seiten unabhängig gefiltert werden können.

Den **Datumsbereich**: zwei Datumsfelder ("Date From", "Date To") mit denselben Standardwerten wie auf der Audio-Player-Seite (vollständiger Aufnahmezeitraum), aber ebenfalls im AppState getrennt gehalten (`hm_filter_date_from`, `hm_filter_date_to`).
...
### 4.5 ECharts-Heatmap-Konfiguration
...
### 6.3 Autocomplete-Spezies-Suche
...
### 6.5 ECharts-Heatmap – Colormap-Abbildung

ECharts kennt keine Colormap-Namen wie "turbo" oder "viridis". Jede Colormap muss als Farbverlauf definiert werden. Für den ersten Schritt ist eine Mapping-Tabelle ausreichend, die für jede der 16 Colormaps Anfangs- und Endfarbe (oder mehrere Stützstellen) als Hex-Werte definiert:
...
### 6.6 HTML/JS-Player – Übergabe schrittweise generierter Daten
...
## Teil 7: Abgrenzung zu späteren Ausbaustufen
