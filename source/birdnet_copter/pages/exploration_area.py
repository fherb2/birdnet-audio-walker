...
Sections:
  1. DB type selector  (local / global)
  2. Folder View       (FolderTree, read-only for local DB selection)
  3. Database Information
  4. Actualize Species List
  5. Notes
  6. Recording Files table
  7. Species List (AG Grid) + Download
"""
...
async def exploration_area() -> None:
    ...
    def _on_db_type_change(val: str) -> None:
        if val == 'global':
            ...
            global_info_row.set_visibility(True)
            global_path_label.set_text(
                str(state.global_index_path)
                if state.global_index_path
                else '(not configured — set in Hangar)'
            )
        else:
            state.active_db_is_global = False
            folder_section.set_visibility(True)
            global_info_row.set_visibility(False)
        _refresh_db_info()
        _render_recording_files.refresh()
        _render_species_list.refresh()
        _update_sidebar(None)
    ...
    def _active_db() -> Optional[Path]:
        """Return the currently effective DB path (local or global)."""
        if state.active_db_is_global:
            return state.global_index_path
    ...
    def _save_notes() -> None:
        ...
        if db is None:
            return
        ok = set_analysis_config(db, 'user_comment', notes_area.value)
    ...
    with client:
        try:
            ...
            if scope == 'all':
                ...
                raw = await grid.get_client_data(
                    method='filtered_sorted', timeout=6.0
                )
                rows = [
                    {
            ...
            db_stem = (
                state.active_db.parent.name if state.active_db else 'export'
            )
            ui.download(
                buf.getvalue().encode('utf-8'),
                f'species_{db_stem}.csv',
            )
        except Exception as e:
    ...
    with client:
        try:
            ...
            with pd.ExcelWriter(buf, engine='openpyxl') as writer:
                ...
                df.to_excel(
                    writer, sheet_name='Species Table', index=False
                )
